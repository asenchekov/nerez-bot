...
module.exports = {
  respond (message) {
    ...
    if (message.content.split(' ').includes('gametime')
      && mentioned) {
      const { channel } = message
      const date = moment(message.content.slice(message.content.indexOf('gametime') + 9));

      if (date.isValid() && date > moment()) {
        channel.send(`@everyone\n\`\`\`diff\n+ Next GameTime will be ${date.format('LLL')} (GMT)\n\`\`\``)
        ...
        message.reply('Please format properly the date and make sure it\'s in the future');
      }
    ...
    && mentioned) {
      ...
      message.reply('Hello, I\'m nEREZ, write `@nEREZ commands`\nto see what Asen made me do')
    }
  }
}